...
export const Marketing = () => {
    ...
    const [apiKey, setApiKey] = useState('xkeysib-xxxxxxxxxxxxxxxxxxxxxxxx'); // Mock masked key
    const [senderEmail, setSenderEmail] = useState('contato@aurasalon.com');
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="mt-6">
                ...
                {activeTab === 'audiência' && <AudienceList />}
...
const MarketingDashboard = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard icon="fas fa-users" label="Contatos Totais" value="1,240" sub="Todos os Salões" color="indigo" />
        <StatCard icon="fas fa-paper-plane" label="Emails Enviados" value="8,500" sub="Últimos 30 dias" color="emerald" />
        <StatCard icon="fas fa-envelope-open-text" label="Taxa de Abertura" value="24.8%" sub="Média Global" color="amber" />
    </div>
);
...
const CampaignsList = () => {
    // Mock Data
    const campaigns = [
        { id: 1, name: 'Novidades de Dezembro', status: 'Enviado', sent_at: '2023-12-01', open_rate: '28%' },
        { id: 2, name: 'Promoção de Ano Novo', status: 'Agendado', sent_at: '2023-12-31', open_rate: '-' },
        { id: 3, name: 'Lembrete de Agendamento', status: 'Rascunho', sent_at: '-', open_rate: '-' },
    ];
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {campaigns.map((c) => (
                            <tr key={c.id} className="hover:bg-indigo-50/30 transition-colors">
                                ...
                                <td className="p-4 text-sm text-slate-600 font-mono">{c.sent_at}</td>
    ...
    <div className="space-y-4">
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-8 text-center shadow-sm">
            ...
            <p className="text-slate-500 text-sm max-w-md mx-auto">
                ...
                Atualmente existem <strong>1,240 totais</strong> de contatos elegíveis para marketing.
            </p>
        </div>
    </div>
);
